<?php
/**
 * Jaringan Kartika 1 - Data IP & Perangkat Jaringan
 * Enhanced Dynamic Theme v2.0
 * Tabel: jaringan_kartika1 (mirror dari inventaris_kartika1 + ip_address)
 */

include 'koneksi.php';

// Update IP Address di tabel jaringan_kartika1
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_ip_id'])) {
    ...
    $sql = "UPDATE jaringan_kartika1 SET ip_address='$ip_address' WHERE id=$id";
    mysqli_query($conn, $sql);
    header('Location: jaringan_kartika1.php?success=ip_updated');
    exit;
}

// Sinkronisasi Manual: Copy data baru dari inventaris_kartika1 ke jaringan_kartika1
if (isset($_GET['sync'])) {
    $sync_sql = "INSERT INTO jaringan_kartika1 (
    ...
    FROM inventaris_kartika1
    WHERE id NOT IN (SELECT id_asal FROM jaringan_kartika1 WHERE id_asal IS NOT NULL)";
    mysqli_query($conn, $sync_sql);

    // Update data yang sudah ada di jaringan jika ada perubahan di inventaris
    $update_sql = "UPDATE jaringan_kartika1 j
    INNER JOIN inventaris_kartika1 i ON j.id_asal = i.id
    ...
    header('Location: jaringan_kartika1.php?success=sync');
    exit;
}
...
<head>
    ...
    <title>Jaringan Kartika 1 - Data IP & Perangkat</title>
    ...
    <div class="breadcrumb-bar">
        <div class="breadcrumb-inner">
            <div class="breadcrumb">
                ...
                <i class="fas fa-chevron-right"></i>
                <span class="breadcrumb-current">Jaringan Kartika 1</span>
            </div>
        </div>
    </div>
    ...
    <main class="main-content">
        <div class="container">
            <section class="page-header">
                <div class="page-badge"><i class="fas fa-network-wired"></i> Jaringan Kartika 1</div>
                <h1 class="page-title">Data Perangkat <span class="gradient-text">Jaringan & IP Address</span></h1>
                <p class="page-subtitle">Kelola alamat IP perangkat jaringan ruangan Kartika 1. Data perangkat diambil dari tabel <strong>jaringan_kartika1</strong> yang tersinkron dengan inventaris.</p>
            </section>
            ...
            <div class="glass-card">
                <div class="card-header-glass">
                    ...
                    <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                        <a href="?sync=1" class="btn-glass btn-primary-glass" style="padding:10px 16px; font-size:13px;" onclick="return confirm('Sinkronisasi data dari inventaris_kartika1 ke jaringan_kartika1? Data yang sudah ada akan diperbarui.')" title="Sinkronisasi Data dari Inventaris">
                            <i class="fas fa-rotate"></i> Sinkron Data
                        </a>
                        <span class="td-badge badge-cyan"><i class="fas fa-database" style="margin-right:4px;"></i> 
                            <?php 
                            $count = mysqli_query($conn, "SELECT COUNT(*) as total FROM jaringan_kartika1");
                    ...
                    <div class="table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php
                                $no = 1;
                                $query = mysqli_query($conn, "SELECT * FROM jaringan_kartika1 ORDER BY id DESC");
                                ...
                                <tr>
                                    <td colspan="13">
                                        <div class="empty-state">
                                            ...
                                            <p>Data perangkat diambil dari tabel <strong>jaringan_kartika1</strong>. Klik tombol <strong>Sinkron Data</strong> untuk mengisi dari inventaris, atau pastikan trigger database sudah aktif.</p>
