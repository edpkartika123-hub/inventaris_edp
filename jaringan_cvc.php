<?php
/**
 * Jaringan CVC - Data IP & Perangkat Jaringan
 * Enhanced Dynamic Theme v2.0
 * Tabel: jaringan_cvc (mirror dari inventaris_cvc + ip_address)
 */

include 'koneksi.php';

// Buat tabel jaringan_cvc otomatis jika belum ada
mysqli_query($conn, "CREATE TABLE IF NOT EXISTS jaringan_cvc (
    id INT(11) NOT NULL AUTO_INCREMENT,
    id_asal INT(11) DEFAULT NULL COMMENT 'ID referensi dari inventaris_cvc',
...
// Update IP Address di tabel jaringan_cvc
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_ip_id'])) {
    ...
    $sql = "UPDATE jaringan_cvc SET ip_address='$ip_address' WHERE id=$id";
    mysqli_query($conn, $sql);
    header('Location: jaringan_cvc.php?success=ip_updated');
    exit;
}

// Sinkronisasi Manual: Copy data baru dari inventaris_cvc ke jaringan_cvc
if (isset($_GET['sync'])) {
    $sync_sql = "INSERT INTO jaringan_cvc (
    ...
    FROM inventaris_cvc
    WHERE id NOT IN (SELECT id_asal FROM jaringan_cvc WHERE id_asal IS NOT NULL)";
    mysqli_query($conn, $sync_sql);

    // Update data yang sudah ada di jaringan jika ada perubahan di inventaris
    $update_sql = "UPDATE jaringan_cvc j
    INNER JOIN inventaris_cvc i ON j.id_asal = i.id
    ...
    header('Location: jaringan_cvc.php?success=sync');
    exit;
}
...
<head>
    ...
    <title>Jaringan CVC - Data IP & Perangkat</title>
...
<body>
    <div class="loading-screen" id="loadingScreen">
        ...
        <div class="loading-text">MEMUAT JARINGAN CVC...</div>
    </div>
    ...
    <div class="breadcrumb-bar">
        <div class="breadcrumb-inner">
            <div class="breadcrumb">
                ...
                <i class="fas fa-chevron-right"></i>
                <span class="breadcrumb-current">Jaringan CVC</span>
            </div>
        </div>
    </div>
    ...
    <main class="main-content">
        <div class="container">
            <section class="page-header">
                <div class="page-badge"><i class="fas fa-network-wired"></i> Jaringan CVC</div>
                <h1 class="page-title">Data Perangkat <span class="gradient-text">Jaringan & IP Address</span></h1>
                <p class="page-subtitle">Kelola alamat IP perangkat jaringan ruangan CVC. Data perangkat diambil dari tabel <strong>jaringan_cvc</strong> yang tersinkron dengan inventaris.</p>
            </section>
            ...
            <div class="glass-card">
                <div class="card-header-glass">
                    ...
                    <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                        <a href="?sync=1" class="btn-glass btn-primary-glass" style="padding:10px 16px; font-size:13px;" onclick="return confirm('Sinkronisasi data dari inventaris_cvc ke jaringan_cvc? Data yang sudah ada akan diperbarui.')" title="Sinkronisasi Data dari Inventaris">
                            <i class="fas fa-rotate"></i> Sinkron Data
                        </a>
                        <span class="td-badge badge-cyan"><i class="fas fa-database" style="margin-right:4px;"></i> 
                            <?php 
                            $count = mysqli_query($conn, "SELECT COUNT(*) as total FROM jaringan_cvc");
                    ...
                    <div class="table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php
                                $no = 1;
                                $query = mysqli_query($conn, "SELECT * FROM jaringan_cvc ORDER BY id DESC");
                                ...
                                <tr>
                                    <td colspan="13">
                                        <div class="empty-state">
                                            ...
                                            <p>Data perangkat diambil dari tabel <strong>jaringan_cvc</strong>. Klik tombol <strong>Sinkron Data</strong> untuk mengisi dari inventaris, atau pastikan trigger database sudah aktif.</p>
