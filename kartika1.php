<?php
/**
 * Kartika 1 - Data Inventaris Hardware & PC EDP
 * Enhanced Dynamic Theme v2.0
 */

include 'koneksi.php';

if (!function_exists('extractYear')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id']) && !isset($_POST['ganti_id']) && !isset($_POST['disposal_id'])) {
    ...
    $sql = "INSERT INTO inventaris_kartika1 (gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp) VALUES ('$gedung', '$lantai', '$ruangan', '$nama_pc', '$nama_user', '$manufactur', '$processor_type', '$windows_version', '$ram_size', '$ram_hardisk', '$monitor_model', '$monitor_size', '$monitor_vga', '$printer_1', '$printer_2', '$tahun_pengadaan', '$id_personil_edp')";
    mysqli_query($conn, $sql);
    header('Location: kartika1.php'); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ganti_id'])) {
    ...
    $query_data = mysqli_query($conn, "SELECT * FROM inventaris_kartika1 WHERE id = $id LIMIT 1");
    ...
    try {
        if ($tipe_ganti === 'pc') {
            ...
            $sql_gudang = "INSERT INTO gudang_kartika1 (id_asal, tipe_ganti, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_ganti, status_gudang) VALUES ($id, '$tipe_ganti', '$gedung', '$lantai', '$ruangan', '$nama_pc', '$nama_user', '$manufactur', '$processor_type', '$windows_version', '$ram_size', '$ram_hardisk', '$monitor_model', '$monitor_size', '$monitor_vga', '', '', '$tahun_pengadaan', '$id_personil_edp', '$alasan', '$status_gudang')";
            if (!mysqli_query($conn, $sql_gudang)) throw new Exception('Gagal pindah ke gudang: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_kartika1 SET nama_pc='', nama_user='', manufactur='', processor_type='', windows_version='', ram_size='', ram_hardisk='', monitor_model='', monitor_size='', monitor_vga='', tahun_pengadaan='', id_personil_edp='' WHERE id = $id";
        ...
        elseif ($tipe_ganti === 'printer1') {
            ...
            $sql_update = "UPDATE inventaris_kartika1 SET printer_1 = '' WHERE id = $id";
        ...
        elseif ($tipe_ganti === 'printer2') {
            ...
            if (!mysqli_query($conn, $sql_gudang)) throw new Exception('Gagal pindah: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_kartika1 SET printer_2 = '' WHERE id = $id";
            if (!mysqli_query($conn, $sql_update)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        elseif ($tipe_ganti === 'semua') {
            ...
            $sql_delete = "DELETE FROM inventaris_kartika1 WHERE id = $id";
            if (!mysqli_query($conn, $sql_delete)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        mysqli_commit($conn);
        header('Location: kartika1.php?success=ganti'); exit;
    } catch (Exception $e) {
        mysqli_rollback($conn);
        die('<div style="font-family:Inter,system-ui;padding:30px;background:#fee2e2;color:#991b1b;max-width:700px;margin:50px auto;border-radius:16px;box-shadow:0 20px 40px rgba(0,0,0,.2);"><h3><i class="fas fa-circle-xmark"></i> Gagal Memindahkan Barang</h3><p>' . htmlspecialchars($e->getMessage()) . '</p><a href="kartika1.php" style="display:inline-flex;align-items:center;gap:8px;margin-top:16px;padding:10px 20px;background:#dc2626;color:#fff;border-radius:10px;text-decoration:none;font-weight:600;"><i class="fas fa-arrow-left"></i> Kembali</a></div>');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['disposal_id'])) {
    ...
    try {
        if ($tipe_disposal === 'pc') {
            ...
            $monitor_size = mysqli_real_escape_string($conn, $data['monitor_size'] ?? '');
            $monitor_vga = mysqli_real_escape_string($conn, $data['monitor_vga'] ?? '');
            $sql_disposal = "INSERT INTO disposal_kartika1 (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '$nama_pc', '$nama_user', '$manufactur', '$processor_type', '$windows_version', '$ram_size', '$ram_hardisk', '$monitor_model', '$monitor_size', '$monitor_vga', '', '', '$tahun_pengadaan', '$id_personil_edp', '$alasan')";
        ...
        elseif ($tipe_disposal === 'printer1') {
            if (trim($data['printer_1'] ?? '') === '') throw new Exception('Printer 1 kosong.');
            $printer_1 = mysqli_real_escape_string($conn, $data['printer_1']);
            $sql_disposal = "INSERT INTO disposal_kartika1 (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '', '', '', '', '', '', '', '', '', '', '$printer_1', '', '', '', '$alasan')";
            if (!mysqli_query($conn, $sql_disposal)) throw new Exception('Gagal disposal: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_kartika1 SET printer_1 = '' WHERE id = $id";
            if (!mysqli_query($conn, $sql_update)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        elseif ($tipe_disposal === 'printer2') {
            if (trim($data['printer_2'] ?? '') === '') throw new Exception('Printer 2 kosong.');
            $printer_2 = mysqli_real_escape_string($conn, $data['printer_2']);
            $sql_disposal = "INSERT INTO disposal_kartika1 (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '', '', '', '', '', '', '', '', '', '', '', '$printer_2', '', '', '$alasan')";
            if (!mysqli_query($conn, $sql_disposal)) throw new Exception('Gagal disposal: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_kartika1 SET printer_2 = '' WHERE id = $id";
            if (!mysqli_query($conn, $sql_update)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        elseif ($tipe_disposal === 'semua') {
            ...
            $escaped = [];
            foreach ($fields as $field) $escaped[$field] = mysqli_real_escape_string($conn, $data[$field] ?? '');
            $sql_disposal = "INSERT INTO disposal_kartika1 (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '{$escaped['nama_pc']}', '{$escaped['nama_user']}', '{$escaped['manufactur']}', '{$escaped['processor_type']}', '{$escaped['windows_version']}', '{$escaped['ram_size']}', '{$escaped['ram_hardisk']}', '{$escaped['monitor_model']}', '{$escaped['monitor_size']}', '{$escaped['monitor_vga']}', '{$escaped['printer_1']}', '{$escaped['printer_2']}', '$tahun_pengadaan', '$id_personil_edp', '$alasan')";
            if (!mysqli_query($conn, $sql_disposal)) throw new Exception('Gagal disposal: ' . mysqli_error($conn));
            $sql_delete = "DELETE FROM inventaris_kartika1 WHERE id = $id";
            if (!mysqli_query($conn, $sql_delete)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        mysqli_commit($conn);
        header('Location: kartika1.php?success=disposal'); exit;
    } catch (Exception $e) {
        mysqli_rollback($conn);
...
if (isset($_GET['hapus'])) {
    ...
    $cek = mysqli_query($conn, "SELECT id FROM inventaris_kartika1 WHERE id=$id LIMIT 1");
    if (mysqli_num_rows($cek) > 0) {
        mysqli_query($conn, "DELETE FROM inventaris_kartika1 WHERE id=$id");
        header('Location: kartika1.php?success=hapus'); exit;
    } else {
        header('Location: kartika1.php?error=notfound'); exit;
    }
}
?>
...
<head>
    ...
    <title>Kartika 1 - Inventaris Hardware & PC EDP</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        html.light-mode {
            ...
            --glass-border-hover: rgba(15,23,42,0.22);
            --text-primary: #0f172a;
            ...
            --input-focus-bg: rgba(2,132,199,0.05);
            --row-bg: rgba(15,23,42,0.02);
            --row-bg-alt: rgba(15,23,42,0.045);
            --row-hover: rgba(2,132,199,0.07);
            --info-box-bg: rgba(15,23,42,0.04);
            --option-bg: rgba(15,23,42,0.04);
            --fade-color: rgba(238,242,247,0.7);
        }
        html.light-mode input[type="date"].form-input { color-scheme: light; }
        html.light-mode input[type="date"].form-input::-webkit-calendar-picker-indicator { filter: none; }
        ...
        /* Header */
        .header { position: fixed; top: 0; left: 0; right: 0; z-index: 1000; background: var(--header-bg); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-bottom: 1px solid var(--glass-border); transition: var(--transition-base); }
        .header.scrolled { background: var(--header-bg-scrolled); box-shadow: var(--shadow-lg); }
        ...
        .menu-btn { width: 44px; height: 44px; border-radius: var(--radius-md); background: var(--btn-ghost-bg); border: 1px solid var(--glass-border); color: var(--text-primary); font-size: 18px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: var(--transition-base); }
        .menu-btn:hover { background: var(--btn-ghost-hover); border-color: var(--glass-border-hover); transform: scale(1.05); }
        ...
        .header-btn { width: 44px; height: 44px; border-radius: var(--radius-md); background: var(--btn-ghost-bg); border: 1px solid var(--glass-border); color: var(--text-primary); font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: var(--transition-base); position: relative; overflow: hidden; }
        ...
        .form-input { width: 100%; padding: 12px 16px; background: var(--input-bg); border: 1px solid var(--glass-border); border-radius: var(--radius-md); color: var(--text-primary); font-size: 14px; font-family: 'Inter', sans-serif; transition: var(--transition-base); outline: none; }
        .form-input::placeholder { color: var(--text-dim); opacity: 0.6; }
        .form-input:focus { background: var(--input-focus-bg); border-color: rgba(56,189,248,0.4); box-shadow: 0 0 0 3px rgba(56,189,248,0.1); }
        ...
        .btn-back-glass { background: var(--btn-ghost-bg); border: 1px solid var(--glass-border); color: var(--text-secondary); padding: 10px 18px; font-size: 13px; }
        .btn-back-glass:hover { background: var(--btn-ghost-hover); color: var(--text-primary); border-color: var(--glass-border-hover); }
        ...
        .table-wrapper::-webkit-scrollbar-track { background: rgba(255,255,255,0.04); border-radius: 4px; }
        .table-wrapper::-webkit-scrollbar-thumb { background: linear-gradient(90deg, var(--accent-blue), var(--accent-cyan)); border-radius: 4px; }
        .scroll-hint-k1 {
            display: none; align-items: center; gap: 8px;
            padding: 10px 24px; font-size: 0.78rem; color: var(--text-muted);
            background: rgba(56,189,248,0.07); border-bottom: 1px solid var(--glass-border);
            animation: fadeInDown 0.4s ease-out;
        }
        .scroll-hint-k1 i { animation: slide-hint-k1 1.2s ease-in-out infinite; color: var(--accent-cyan); }
        @keyframes slide-hint-k1 { 0%,100% { transform: translateX(0); } 50% { transform: translateX(6px); } }
        .table-fade-k1 { position: relative; }
        .table-fade-k1::after {
            content: ''; position: absolute; top: 0; right: 0; width: 45px; height: 100%;
            background: linear-gradient(90deg, transparent, var(--fade-color));
            pointer-events: none; transition: opacity 0.3s;
        }
        .table-fade-k1.scrolled-end::after { opacity: 0; }
        .data-table { width: 100%; min-width: 1900px; border-collapse: separate; border-spacing: 0; font-size: 12px; }
        .data-table th:nth-child(1),  .data-table td:nth-child(1)  { min-width: 55px; }
        .data-table th:nth-child(2),  .data-table td:nth-child(2)  { min-width: 100px; }
        .data-table th:nth-child(3),  .data-table td:nth-child(3)  { min-width: 60px; }
        .data-table th:nth-child(4),  .data-table td:nth-child(4)  { min-width: 110px; }
        ...
        .data-table th:nth-child(7),  .data-table td:nth-child(7)  { min-width: 100px; }
        .data-table th:nth-child(8),  .data-table td:nth-child(8)  { min-width: 140px; }
        .data-table th:nth-child(9),  .data-table td:nth-child(9)  { min-width: 120px; }
        .data-table th:nth-child(10), .data-table td:nth-child(10) { min-width: 90px; }
        .data-table th:nth-child(11), .data-table td:nth-child(11) { min-width: 100px; }
        .data-table th:nth-child(12), .data-table td:nth-child(12) { min-width: 120px; }
        .data-table th:nth-child(13), .data-table td:nth-child(13) { min-width: 80px; }
        .data-table th:nth-child(14), .data-table td:nth-child(14) { min-width: 100px; }
        ...
        .data-table th:nth-child(17), .data-table td:nth-child(17) { min-width: 90px; }
        .data-table th:nth-child(18), .data-table td:nth-child(18) { min-width: 90px; }
        .data-table th:nth-child(19), .data-table td:nth-child(19) { min-width: 320px; }
        ...
        .data-table tbody tr { background: var(--row-bg); transition: var(--transition-fast); animation: fadeInUp 0.4s ease both; }
        ...
        .data-table tbody tr:nth-child(even) { background: var(--row-bg-alt); }
        ...
        .modal-close { width: 36px; height: 36px; border-radius: var(--radius-md); background: var(--btn-ghost-bg); border: 1px solid var(--glass-border); color: var(--text-muted); font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: var(--transition-base); }
        ...
        .info-box { background: var(--info-box-bg); border: 1px solid var(--glass-border); border-radius: var(--radius-md); padding: 16px; margin-bottom: 20px; }
        .info-box p { color: var(--text-muted); font-size: 13px; line-height: 1.6; margin: 0; }
        .info-box strong { color: var(--text-primary); }
        .info-box i { margin-right: 6px; }

        /* Options */
        .ganti-option, .disposal-option { display: flex; align-items: center; gap: 10px; cursor: pointer; color: var(--text-secondary); font-size: 14px; padding: 12px 16px; background: var(--option-bg); border: 1px solid var(--glass-border); border-radius: var(--radius-md); transition: var(--transition-base); font-weight: 600; }
        ...
        @media (max-width: 767px) {
            ...
            .table-wrapper { margin: 0 -4px; padding: 0 4px; }
            .data-table { min-width: 1700px; font-size: 11px; }
            .data-table thead th, .data-table tbody td { padding: 10px 8px; }
            .data-table th:nth-child(19), .data-table td:nth-child(19) { min-width: 300px; }
    ...
    <script>
        ...
        (function(){ if (localStorage.getItem('theme') === 'light') { document.documentElement.classList.add('light-mode'); } })();
    </script>
</head>
...
<body>
    <div class="loading-screen" id="loadingScreen">
        ...
        <div class="loading-text">MEMUAT KARTIKA 1...</div>
    </div>
    ...
    <header class="header" id="header">
        <div class="header-inner">
            ...
            <div class="header-right">
                ...
                <button class="header-btn" id="themeToggle" onclick="toggleTheme()" title="Ganti Tema Gelap / Terang"><i class="fas fa-sun" id="themeIcon"></i></button>
    ...
    <div class="breadcrumb-bar">
        <div class="breadcrumb-inner">
            <div class="breadcrumb">
                ...
                <i class="fas fa-chevron-right"></i>
                <span class="breadcrumb-current">Kartika 1</span>
            </div>
        </div>
    </div>
    ...
    <main class="main-content">
        <div class="container">
            <section class="page-header">
                <div class="page-badge"><i class="fas fa-door-open"></i> Ruangan Kartika 1</div>
                <h1 class="page-title">Data Inventaris <span class="gradient-text">Hardware & PC EDP</span></h1>
                <p class="page-subtitle">Input dan kelola data inventaris komputer, monitor, printer, dan perangkat EDP ruangan Kartika 1.</p>
            </section>
            ...
            <div class="glass-card">
                <div class="card-header-glass">
                    <div class="card-header-title"><i class="fas fa-table-list"></i> Daftar Inventaris Kartika 1</div>
                    <span class="td-badge badge-cyan"><i class="fas fa-database" style="margin-right:4px;"></i> 
                        <?php 
                        $count = mysqli_query($conn, "SELECT COUNT(*) as total FROM inventaris_kartika1");
                ...
                <div class="card-body-glass">
                    <div class="scroll-hint-k1" id="scrollHintK1"><i class="fas fa-arrows-left-right"></i> Geser ke samping untuk melihat semua kolom</div>
                    <div class="table-fade-k1" id="tableFadeK1">
                    <div class="table-wrapper" id="tableWrapK1">
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php
                                $no = 1;
                                $query = mysqli_query($conn, "SELECT * FROM inventaris_kartika1 ORDER BY id DESC");
                                ...
                                <tr>
                                    ...
                                    <td data-label="Aksi">
                                        <div class="btn-action-group">
                                            ...
                                            <a href="?hapus=<?= $row['id']; ?>" class="btn-glass btn-danger-glass action-icon-btn hapus-btn" style="background: linear-gradient(135deg, #7f1d1d, #991b1b); text-decoration: none;" onclick="return confirmHapus(event, <?= $row['id']; ?>)" title="Hapus Permanen">
                                                <i class="fas fa-trash-can"></i><span class="action-text">Hapus</span>
                                            </a>
                                        </div>
                                    </td>
                                </tr>
...
var inventarisData = [
<?php
$query_js = mysqli_query($conn, "SELECT * FROM inventaris_kartika1 ORDER BY id DESC");
$js_rows = [];
...
?>
            ...
            <form method="POST" action="" id="gantiForm">
                ...
                <div class="modal-body">
                    <div class="info-box" style="border-color: rgba(56,189,248,0.3);">
                        ...
                        <strong>Perhatian!</strong> Pilih barang yang akan <strong>dipindahkan ke Gudang</strong>. Barang lain tetap tinggal di inventaris aktif.</p>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn-glass btn-info-glass" onclick="return confirm('Yakin ingin memindahkan barang yang dipilih ke gudang?')"><i class="fas fa-warehouse"></i> Pindahkan ke Perbaikan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleTheme() {
            ...
            applyTheme(next);
            showToast('Tema ' + (next === 'light' ? 'Terang' : 'Gelap') + ' aktif', 'info');
        }
        // Sinkronkan ikon jika tema sudah diterapkan oleh script di <head>
        ...
        (function() {
            ...
            if (success === 'hapus') showToast('Data berhasil dihapus secara permanen!', 'success');
            if (error === 'notfound') showToast('Data tidak ditemukan!', 'error');
            if (window.history.replaceState && (success || error)) {
        ...
        (function() {
            const wrap = document.getElementById('tableWrapK1');
            const hint = document.getElementById('scrollHintK1');
            const fade = document.getElementById('tableFadeK1');
            if (!wrap) return;
