...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    $sql = "UPDATE inventaris_cicu SET gedung='$gedung', lantai='$lantai', ruangan='$ruangan', nama_pc='$nama_pc', nama_user='$nama_user', manufactur='$manufactur', processor_type='$processor_type', windows_version='$windows_version', ram_size='$ram_size', ram_hardisk='$ram_hardisk', monitor_model='$monitor_model', monitor_size='$monitor_size', monitor_vga='$monitor_vga', printer_1='$printer_1', printer_2='$printer_2', tahun_pengadaan='$tahun_pengadaan', id_personil_edp='$id_personil_edp' WHERE id=$id";
    mysqli_query($conn, $sql);
    header('Location: cicu.php'); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ganti_id'])) {
    ...
    $query_data = mysqli_query($conn, "SELECT * FROM inventaris_cicu WHERE id = $id LIMIT 1");
    ...
    try {
        if ($tipe_ganti === 'pc') {
            ...
            $sql_gudang = "INSERT INTO gudang_cicu (id_asal, tipe_ganti, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_ganti, status_gudang) VALUES ($id, '$tipe_ganti', '$gedung', '$lantai', '$ruangan', '$nama_pc', '$nama_user', '$manufactur', '$processor_type', '$windows_version', '$ram_size', '$ram_hardisk', '$monitor_model', '$monitor_size', '$monitor_vga', '', '', '$tahun_pengadaan', '$id_personil_edp', '$alasan', '$status_gudang')";
            if (!mysqli_query($conn, $sql_gudang)) throw new Exception('Gagal pindah ke gudang: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_cicu SET nama_pc='', nama_user='', manufactur='', processor_type='', windows_version='', ram_size='', ram_hardisk='', monitor_model='', monitor_size='', monitor_vga='', tahun_pengadaan='', id_personil_edp='' WHERE id = $id";
        ...
        elseif ($tipe_ganti === 'printer1') {
            ...
            $sql_update = "UPDATE inventaris_cicu SET printer_1 = '' WHERE id = $id";
        ...
        elseif ($tipe_ganti === 'printer2') {
            ...
            if (!mysqli_query($conn, $sql_gudang)) throw new Exception('Gagal pindah: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_cicu SET printer_2 = '' WHERE id = $id";
            if (!mysqli_query($conn, $sql_update)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['disposal_id'])) {
    ...
    try {
        if ($tipe_disposal === 'pc') {
            ...
            $monitor_size = mysqli_real_escape_string($conn, $data['monitor_size'] ?? '');
            $monitor_vga = mysqli_real_escape_string($conn, $data['monitor_vga'] ?? '');
            $sql_disposal = "INSERT INTO disposal_cicu (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '$nama_pc', '$nama_user', '$manufactur', '$processor_type', '$windows_version', '$ram_size', '$ram_hardisk', '$monitor_model', '$monitor_size', '$monitor_vga', '', '', '$tahun_pengadaan', '$id_personil_edp', '$alasan')";
        ...
        elseif ($tipe_disposal === 'printer1') {
            if (trim($data['printer_1'] ?? '') === '') throw new Exception('Printer 1 kosong.');
            $printer_1 = mysqli_real_escape_string($conn, $data['printer_1']);
            $sql_disposal = "INSERT INTO disposal_cicu (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '', '', '', '', '', '', '', '', '', '', '$printer_1', '', '', '', '$alasan')";
            if (!mysqli_query($conn, $sql_disposal)) throw new Exception('Gagal disposal: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_cicu SET printer_1 = '' WHERE id = $id";
            if (!mysqli_query($conn, $sql_update)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        elseif ($tipe_disposal === 'printer2') {
            if (trim($data['printer_2'] ?? '') === '') throw new Exception('Printer 2 kosong.');
            $printer_2 = mysqli_real_escape_string($conn, $data['printer_2']);
            $sql_disposal = "INSERT INTO disposal_cicu (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '', '', '', '', '', '', '', '', '', '', '', '$printer_2', '', '', '$alasan')";
            if (!mysqli_query($conn, $sql_disposal)) throw new Exception('Gagal disposal: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_cicu SET printer_2 = '' WHERE id = $id";
            if (!mysqli_query($conn, $sql_update)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        elseif ($tipe_disposal === 'semua') {
            ...
            $escaped = [];
            foreach ($fields as $field) $escaped[$field] = mysqli_real_escape_string($conn, $data[$field] ?? '');
            $sql_disposal = "INSERT INTO disposal_cicu (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '{$escaped['nama_pc']}', '{$escaped['nama_user']}', '{$escaped['manufactur']}', '{$escaped['processor_type']}', '{$escaped['windows_version']}', '{$escaped['ram_size']}', '{$escaped['ram_hardisk']}', '{$escaped['monitor_model']}', '{$escaped['monitor_size']}', '{$escaped['monitor_vga']}', '{$escaped['printer_1']}', '{$escaped['printer_2']}', '$tahun_pengadaan', '$id_personil_edp', '$alasan')";
            if (!mysqli_query($conn, $sql_disposal)) throw new Exception('Gagal disposal: ' . mysqli_error($conn));
            $sql_delete = "DELETE FROM inventaris_cicu WHERE id = $id";
            if (!mysqli_query($conn, $sql_delete)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        mysqli_commit($conn);
        header('Location: cicu.php?success=disposal'); exit;
    } catch (Exception $e) {
        mysqli_rollback($conn);
...
if (isset($_GET['hapus'])) {
    ...
    mysqli_query($conn, "DELETE FROM inventaris_cicu WHERE id=$id");
    header('Location: cicu.php'); exit;
}
?>
...
<head>
    ...
    <title>CICU - Inventaris Hardware & PC EDP</title>
...
<body>
    <div class="loading-screen" id="loadingScreen">
        ...
        <div class="loading-text">MEMUAT CICU...</div>
    </div>
    ...
    <div class="breadcrumb-bar">
        <div class="breadcrumb-inner">
            <div class="breadcrumb">
                ...
                <i class="fas fa-chevron-right"></i>
                <span class="breadcrumb-current">CICU</span>
            </div>
        </div>
    </div>
    ...
    <main class="main-content">
        <div class="container">
            <section class="page-header">
                <div class="page-badge"><i class="fas fa-door-open"></i> Ruangan CICU</div>
            ...
            <div class="glass-card">
                <div class="card-header-glass">
                    <div class="card-header-title"><i class="fas fa-table-list"></i> Daftar Inventaris CICU</div>
                    <span class="td-badge badge-cyan"><i class="fas fa-database" style="margin-right:4px;"></i> 
                        <?php 
                        $count = mysqli_query($conn, "SELECT COUNT(*) as total FROM inventaris_cicu");
                    ...
                    <div class="table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php
                                $no = 1;
                                $query = mysqli_query($conn, "SELECT * FROM inventaris_cicu ORDER BY id DESC");
...
var inventarisData = [
<?php
$query_js = mysqli_query($conn, "SELECT * FROM inventaris_cicu ORDER BY id DESC");
$js_rows = [];
