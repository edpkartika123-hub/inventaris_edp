<?php
/**
 * Jaringan Kartika 2 - Data IP & Perangkat Jaringan
 * Enhanced Dynamic Theme v2.0
 * Tabel: jaringan_kartika2 (mirror dari inventaris_kartika2 + ip_address)
 */

include 'koneksi.php';

// Buat tabel jaringan_kartika2 otomatis jika belum ada
mysqli_query($conn, "CREATE TABLE IF NOT EXISTS jaringan_kartika2 (
    id INT AUTO_INCREMENT PRIMARY KEY,
    id_asal INT DEFAULT NULL,
    gedung VARCHAR(100) DEFAULT NULL,
    lantai VARCHAR(50) DEFAULT NULL,
    ruangan VARCHAR(100) DEFAULT NULL,
    nama_pc VARCHAR(150) DEFAULT NULL,
    nama_user VARCHAR(150) DEFAULT NULL,
    manufactur VARCHAR(100) DEFAULT NULL,
    processor_type VARCHAR(100) DEFAULT NULL,
    windows_version VARCHAR(100) DEFAULT NULL,
    ram_size VARCHAR(50) DEFAULT NULL,
    ram_hardisk VARCHAR(100) DEFAULT NULL,
    monitor_model VARCHAR(100) DEFAULT NULL,
    monitor_size VARCHAR(50) DEFAULT NULL,
    monitor_vga VARCHAR(100) DEFAULT NULL,
    printer_1 VARCHAR(150) DEFAULT NULL,
    printer_2 VARCHAR(150) DEFAULT NULL,
    tahun_pengadaan VARCHAR(20) DEFAULT NULL,
    id_personil_edp INT DEFAULT NULL,
    ip_address VARCHAR(50) DEFAULT NULL,
    INDEX idx_id_asal (id_asal)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");

// Update IP Address di tabel jaringan_kartika2
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_ip_id'])) {
    ...
    $sql = "UPDATE jaringan_kartika2 SET ip_address='$ip_address' WHERE id=$id";
    mysqli_query($conn, $sql);
    header('Location: jaringan_kartika2.php?success=ip_updated');
    exit;
}

// Sinkronisasi Manual: Copy data baru dari inventaris_kartika2 ke jaringan_kartika2
if (isset($_GET['sync'])) {
    $sync_sql = "INSERT INTO jaringan_kartika2 (
    ...
    WHERE id NOT IN (SELECT id_asal FROM jaringan_kartika2 WHERE id_asal IS NOT NULL)";
    mysqli_query($conn, $sync_sql);

    // Update data yang sudah ada di jaringan jika ada perubahan di inventaris
    $update_sql = "UPDATE jaringan_kartika2 j
    INNER JOIN inventaris_kartika2 i ON j.id_asal = i.id
    ...
    header('Location: jaringan_kartika2.php?success=sync');
    exit;
}
...
<head>
    ...
    <title>Jaringan Kartika 2 - Data IP & Perangkat</title>
...
<body>
    <div class="loading-screen" id="loadingScreen">
        ...
        <div class="loading-text">MEMUAT JARINGAN KARTIKA 1...</div>
    </div>
    ...
    <div class="breadcrumb-bar">
        <div class="breadcrumb-inner">
            <div class="breadcrumb">
                ...
                <i class="fas fa-chevron-right"></i>
                <span class="breadcrumb-current">Jaringan Kartika 2</span>
            </div>
        </div>
    </div>
    ...
    <main class="main-content">
        <div class="container">
            <section class="page-header">
                <div class="page-badge"><i class="fas fa-network-wired"></i> Jaringan Kartika 2</div>
                <h1 class="page-title">Data Perangkat <span class="gradient-text">Jaringan & IP Address</span></h1>
                <p class="page-subtitle">Kelola alamat IP perangkat jaringan ruangan Kartika 2. Data perangkat diambil dari tabel <strong>jaringan_kartika2</strong> yang tersinkron dengan inventaris.</p>
            </section>
            ...
            <div class="glass-card">
                <div class="card-header-glass">
                    ...
                    <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                        <a href="?sync=1" class="btn-glass btn-primary-glass" style="padding:10px 16px; font-size:13px;" onclick="return confirm('Sinkronisasi data dari inventaris_kartika2 ke jaringan_kartika2? Data yang sudah ada akan diperbarui.')" title="Sinkronisasi Data dari Inventaris">
                            <i class="fas fa-rotate"></i> Sinkron Data
                        </a>
                        <span class="td-badge badge-cyan"><i class="fas fa-database" style="margin-right:4px;"></i> 
                            <?php 
                            $count = mysqli_query($conn, "SELECT COUNT(*) as total FROM jaringan_kartika2");
                    ...
                    <div class="table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php
                                $no = 1;
                                $query = mysqli_query($conn, "SELECT * FROM jaringan_kartika2 ORDER BY id DESC");
                                ...
                                <tr>
                                    <td colspan="13">
                                        <div class="empty-state">
                                            ...
                                            <p>Data perangkat diambil dari tabel <strong>jaringan_kartika2</strong>. Klik tombol <strong>Sinkron Data</strong> untuk mengisi dari inventaris, atau pastikan trigger database sudah aktif.</p>
