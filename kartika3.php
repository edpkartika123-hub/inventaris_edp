...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id']) && !isset($_POST['ganti_id']) && !isset($_POST['disposal_id'])) {
    ...
    $sql = "INSERT INTO inventaris_kartika3 (gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp) VALUES ('$gedung', '$lantai', '$ruangan', '$nama_pc', '$nama_user', '$manufactur', '$processor_type', '$windows_version', '$ram_size', '$ram_hardisk', '$monitor_model', '$monitor_size', '$monitor_vga', '$printer_1', '$printer_2', '$tahun_pengadaan', '$id_personil_edp')";
    mysqli_query($conn, $sql);
    header('Location: kartika3.php'); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    $sql = "UPDATE inventaris_kartika3 SET gedung='$gedung', lantai='$lantai', ruangan='$ruangan', nama_pc='$nama_pc', nama_user='$nama_user', manufactur='$manufactur', processor_type='$processor_type', windows_version='$windows_version', ram_size='$ram_size', ram_hardisk='$ram_hardisk', monitor_model='$monitor_model', monitor_size='$monitor_size', monitor_vga='$monitor_vga', printer_1='$printer_1', printer_2='$printer_2', tahun_pengadaan='$tahun_pengadaan', id_personil_edp='$id_personil_edp' WHERE id=$id";
    mysqli_query($conn, $sql);
    header('Location: kartika3.php'); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ganti_id'])) {
    ...
    $query_data = mysqli_query($conn, "SELECT * FROM inventaris_kartika3 WHERE id = $id LIMIT 1");
    ...
    try {
        if ($tipe_ganti === 'pc') {
            ...
            $sql_gudang = "INSERT INTO gudang_kartika3 (id_asal, tipe_ganti, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_ganti, status_gudang) VALUES ($id, '$tipe_ganti', '$gedung', '$lantai', '$ruangan', '$nama_pc', '$nama_user', '$manufactur', '$processor_type', '$windows_version', '$ram_size', '$ram_hardisk', '$monitor_model', '$monitor_size', '$monitor_vga', '', '', '$tahun_pengadaan', '$id_personil_edp', '$alasan', '$status_gudang')";
            if (!mysqli_query($conn, $sql_gudang)) throw new Exception('Gagal pindah ke gudang: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_kartika3 SET nama_pc='', nama_user='', manufactur='', processor_type='', windows_version='', ram_size='', ram_hardisk='', monitor_model='', monitor_size='', monitor_vga='', tahun_pengadaan='', id_personil_edp='' WHERE id = $id";
        ...
        elseif ($tipe_ganti === 'printer1') {
            ...
            $sql_update = "UPDATE inventaris_kartika3 SET printer_1 = '' WHERE id = $id";
        ...
        elseif ($tipe_ganti === 'printer2') {
            ...
            if (!mysqli_query($conn, $sql_gudang)) throw new Exception('Gagal pindah: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_kartika3 SET printer_2 = '' WHERE id = $id";
            if (!mysqli_query($conn, $sql_update)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        elseif ($tipe_ganti === 'semua') {
            ...
            $sql_delete = "DELETE FROM inventaris_kartika3 WHERE id = $id";
            if (!mysqli_query($conn, $sql_delete)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        mysqli_commit($conn);
        header('Location: kartika3.php?success=ganti'); exit;
    } catch (Exception $e) {
        mysqli_rollback($conn);
        die('<div style="font-family:Inter,system-ui;padding:30px;background:#fee2e2;color:#991b1b;max-width:700px;margin:50px auto;border-radius:16px;box-shadow:0 20px 40px rgba(0,0,0,.2);"><h3><i class="fas fa-circle-xmark"></i> Gagal Memindahkan Barang</h3><p>' . htmlspecialchars($e->getMessage()) . '</p><a href="kartika3.php" style="display:inline-flex;align-items:center;gap:8px;margin-top:16px;padding:10px 20px;background:#dc2626;color:#fff;border-radius:10px;text-decoration:none;font-weight:600;"><i class="fas fa-arrow-left"></i> Kembali</a></div>');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['disposal_id'])) {
    ...
    try {
        if ($tipe_disposal === 'pc') {
            ...
            $monitor_size = mysqli_real_escape_string($conn, $data['monitor_size'] ?? '');
            $monitor_vga = mysqli_real_escape_string($conn, $data['monitor_vga'] ?? '');
            $sql_disposal = "INSERT INTO disposal_kartika3 (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '$nama_pc', '$nama_user', '$manufactur', '$processor_type', '$windows_version', '$ram_size', '$ram_hardisk', '$monitor_model', '$monitor_size', '$monitor_vga', '', '', '$tahun_pengadaan', '$id_personil_edp', '$alasan')";
        ...
        elseif ($tipe_disposal === 'printer1') {
            if (trim($data['printer_1'] ?? '') === '') throw new Exception('Printer 1 kosong.');
            $printer_1 = mysqli_real_escape_string($conn, $data['printer_1']);
            $sql_disposal = "INSERT INTO disposal_kartika3 (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '', '', '', '', '', '', '', '', '', '', '$printer_1', '', '', '', '$alasan')";
            if (!mysqli_query($conn, $sql_disposal)) throw new Exception('Gagal disposal: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_kartika3 SET printer_1 = '' WHERE id = $id";
            if (!mysqli_query($conn, $sql_update)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        elseif ($tipe_disposal === 'printer2') {
            if (trim($data['printer_2'] ?? '') === '') throw new Exception('Printer 2 kosong.');
            $printer_2 = mysqli_real_escape_string($conn, $data['printer_2']);
            $sql_disposal = "INSERT INTO disposal_kartika3 (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '', '', '', '', '', '', '', '', '', '', '', '$printer_2', '', '', '$alasan')";
            if (!mysqli_query($conn, $sql_disposal)) throw new Exception('Gagal disposal: ' . mysqli_error($conn));
            $sql_update = "UPDATE inventaris_kartika3 SET printer_2 = '' WHERE id = $id";
            if (!mysqli_query($conn, $sql_update)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        elseif ($tipe_disposal === 'semua') {
            ...
            $escaped = [];
            foreach ($fields as $field) $escaped[$field] = mysqli_real_escape_string($conn, $data[$field] ?? '');
            $sql_disposal = "INSERT INTO disposal_kartika3 (id_asal, tipe_disposal, gedung, lantai, ruangan, nama_pc, nama_user, manufactur, processor_type, windows_version, ram_size, ram_hardisk, monitor_model, monitor_size, monitor_vga, printer_1, printer_2, tahun_pengadaan, id_personil_edp, alasan_disposal) VALUES ($id, '$tipe_disposal', '$gedung', '$lantai', '$ruangan', '{$escaped['nama_pc']}', '{$escaped['nama_user']}', '{$escaped['manufactur']}', '{$escaped['processor_type']}', '{$escaped['windows_version']}', '{$escaped['ram_size']}', '{$escaped['ram_hardisk']}', '{$escaped['monitor_model']}', '{$escaped['monitor_size']}', '{$escaped['monitor_vga']}', '{$escaped['printer_1']}', '{$escaped['printer_2']}', '$tahun_pengadaan', '$id_personil_edp', '$alasan')";
            if (!mysqli_query($conn, $sql_disposal)) throw new Exception('Gagal disposal: ' . mysqli_error($conn));
            $sql_delete = "DELETE FROM inventaris_kartika3 WHERE id = $id";
            if (!mysqli_query($conn, $sql_delete)) throw new Exception('Gagal hapus: ' . mysqli_error($conn));
        }
        mysqli_commit($conn);
        header('Location: kartika3.php?success=disposal'); exit;
    } catch (Exception $e) {
        mysqli_rollback($conn);
...
if (isset($_GET['hapus'])) {
    ...
    mysqli_query($conn, "DELETE FROM inventaris_kartika3 WHERE id=$id");
    header('Location: kartika3.php'); exit;
}
?>
...
<head>
    ...
    <title>Kartika 3 - Inventaris Hardware & PC EDP</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .table-wrapper::-webkit-scrollbar-track { background: var(--bg-card); border-radius: var(--radius-md); }
        .table-wrapper::-webkit-scrollbar-thumb { background: linear-gradient(135deg, var(--accent-blue), var(--accent-cyan)); border-radius: var(--radius-md); border: 2px solid var(--bg-card); }
        .table-wrapper::-webkit-scrollbar-thumb:hover { background: linear-gradient(135deg, var(--accent-cyan), var(--accent-blue)); }
        ...
        .table-wrapper.has-scroll-left::before {
            left: 0;
            background: linear-gradient(90deg, rgba(11,17,32,0.9), transparent);
            opacity: 1;
        }
        .table-wrapper.has-scroll-right::after {
            right: 0;
            background: linear-gradient(270deg, rgba(11,17,32,0.9), transparent);
            opacity: 1;
        }
        ...
        .data-table thead th:last-child { border-radius: 0 var(--radius-md) 0 0; border-right: none; }
        ...
        .data-table tbody tr:hover { background: rgba(56,189,248,0.08); }
        .data-table tbody td { 
            padding: 12px 12px; 
        ...
        .data-table tbody tr:nth-child(even) td:first-child { background: rgba(30,41,59,0.95); }
        .data-table tbody tr:hover td:first-child { background: rgba(56,189,248,0.12); }
        ...
        .data-table tbody tr:last-child td:last-child { border-radius: 0 0 var(--radius-md) 0; border-right: none; }
        .data-table tbody td:last-child { border-right: none; }

        .td-center { text-align: center; }
        .td-small { font-size: 12px; color: var(--text-dim); }
        .td-badge { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; border-radius: 50px; font-size: 11px; font-weight: 700; letter-spacing: 0.3px; white-space: nowrap; }
        ...
        @media (max-width: 767px) {
            ...
            /* Excel-like table on mobile - horizontal scroll */
            .table-wrapper { 
                max-height: 60vh;
            ...
            .data-table thead th:first-child { min-width: 45px; }
            .data-table tbody td { 
                padding: 8px 8px; 
                font-size: 11px;
                min-width: 60px;
            }
            .data-table tbody td:first-child { min-width: 45px; }
            .td-badge { font-size: 9px; padding: 2px 6px; }
            .action-icon-btn { width: 32px; height: 32px; }
            .action-icon-btn i { font-size: 12px; }
            .action-icon-btn:hover { width: 32px; padding: 0 !important; transform: none; }
            .action-icon-btn:hover .action-text { display: none; }
            .action-icon-btn.disposal-btn:hover { width: 32px; }
            .sidebar { width: 280px; }
        ...
        @media (max-width: 575px) {
            ...
            .data-table thead th { padding: 8px 6px; font-size: 8px; min-width: 50px; }
            .data-table thead th:first-child { min-width: 38px; }
            .data-table tbody td { padding: 6px 6px; font-size: 10px; min-width: 50px; }
            .data-table tbody td:first-child { min-width: 38px; }
            .td-badge { font-size: 8px; padding: 2px 5px; }
        }
    </style>
</head>
...
<body>
    <div class="loading-screen" id="loadingScreen">
        ...
        <div class="loading-text">MEMUAT KARTIKA 3...</div>
    </div>
    ...
    <div class="breadcrumb-bar">
        <div class="breadcrumb-inner">
            <div class="breadcrumb">
                ...
                <i class="fas fa-chevron-right"></i>
                <span class="breadcrumb-current">Kartika 3</span>
            </div>
        </div>
    </div>
    ...
    <main class="main-content">
        <div class="container">
            <section class="page-header">
                <div class="page-badge"><i class="fas fa-door-open"></i> Ruangan Kartika 3</div>
                <h1 class="page-title">Data Inventaris <span class="gradient-text">Hardware & PC EDP</span></h1>
                <p class="page-subtitle">Input dan kelola data inventaris komputer, monitor, printer, dan perangkat EDP ruangan Kartika 3.</p>
            </section>
            ...
            <div class="glass-card">
                <div class="card-header-glass">
                    <div class="card-header-title"><i class="fas fa-table-list"></i> Daftar Inventaris Kartika 3</div>
                    <span class="td-badge badge-cyan"><i class="fas fa-database" style="margin-right:4px;"></i> 
                        <?php 
                        $count = mysqli_query($conn, "SELECT COUNT(*) as total FROM inventaris_kartika3");
                    ...
                    <div class="table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php
                                $no = 1;
                                $query = mysqli_query($conn, "SELECT * FROM inventaris_kartika3 ORDER BY id DESC");
                                    ...
                                    <td data-label="Aksi">
                                        <div class="btn-action-group">
                                            ...
                                            <button type="button" class="btn-glass btn-info-glass action-icon-btn" onclick="openGantiModal(<?= $row['id']; ?>)" title="Perbaikan / Pindah ke Daftar Perbaikan">
                                                <i class="fas fa-warehouse"></i><span class="action-text">Ganti</span>
                                            </button>
...
var inventarisData = [
<?php
$query_js = mysqli_query($conn, "SELECT * FROM inventaris_kartika3 ORDER BY id DESC");
$js_rows = [];
        ...
        <div class="modal-content">
            ...
            <form method="POST" action="" id="gantiForm">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn-glass btn-info-glass" onclick="return confirm('Yakin ingin memindahkan barang yang dipilih ke gudang?')"><i class="fas fa-warehouse"></i> Pindahkan ke Gudang</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function updateScrollShadows() {
            ...
            wrappers.forEach(wrapper => {
                const hasScrollLeft = wrapper.scrollLeft > 5;
                const hasScrollRight = wrapper.scrollLeft < (wrapper.scrollWidth - wrapper.clientWidth - 5);
        ...
        setTimeout(updateScrollShadows, 500);
